...
- **Auto-check & prompt**: `useFirmwareUpdateCheck.ts` — once opid+fwv are read, `getDeviceSnapshotWithGattMeta(opid)` → `gatt_meta.firmware` names the device's firmware family → catalog entries matched by `actorName` (case-insensitive) → dotted-numeric version compare → accept/decline banner in device details; "Not now" persisted per device+version in localStorage (`otaDismissedVersion_<mac>`). Conservative: ANY missing link (no snapshot, no family, unparseable version) → silent, manual flow only. UNCONFIRMED assumption (ask Bob): `gatt_meta.firmware` ↔ `ItemFirmware.actorName` naming; fix `matchesFirmwareFamily()` if wrong.
- **FTP server** (`ftp.omnivoltaic.com:2021`): standalone, NOT connected to S3/catalog. Usable only as a developer upload inbox IF a backend FTP→S3+catalog sync is built; never as an app-facing delivery path.
