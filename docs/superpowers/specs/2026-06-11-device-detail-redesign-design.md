...
3. **Add Days card (primary)** — accent-bordered card with soft glow (`--accent` border + shadow). Contents: "Add days" label; quick-pick chips **7 / 14 / 30 / 90 / Custom** (Custom reveals the numeric input, same digit-only validation); gradient button **"Generate & Write"**. Disabled when busy, no duration selected, or device unidentified. **Revised 2026-06-11 (Dennis):** in Custom mode the input and button sit side by side on one row (input fixed-width, button flex) instead of stacking — vertical stacking looked off on small phones; the label was shortened from "Generate & Write to Device" to fit.
