...
const FleetsApp = dynamic(() => import('./FleetsApp'), {
...
export default function FleetsPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[Fleets] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/fleets" />
      )}
      {screen === 'selectSA' && (
      ...
      {screen === 'app' && <FleetsApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
