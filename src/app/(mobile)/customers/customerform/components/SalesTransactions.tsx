...
import { RefreshCw, ChevronRight, User, Receipt, Calendar } from 'lucide-react';
    ...
    <div className="attendant-transactions">
      ...
      <div className="transactions-header">
        <div className="transactions-header-top">
          <h2 className="transactions-title">
            {t('sales.transactions.title') || 'My Transactions'}
          </h2>
          <button 
      ...
      {data?.summary && !isLoading && (
        <div className="transactions-summary">
          <div className="summary-card">
            <span className="summary-label">{t('sales.transactions.totalAmount') || 'Total'}</span>
            <span className="summary-value">
              {data.transactions[0]?.currency || ''} {data.summary.total_amount.toLocaleString()}
            </span>
          </div>
          <div className="summary-card">
            <span className="summary-label">{t('sales.transactions.count') || 'Transactions'}</span>
            <span className="summary-value">{data.summary.total_transactions}</span>
          </div>
          <div className="summary-card">
            <span className="summary-label">{t('sales.transactions.customers') || 'Customers'}</span>
      ...
      {!isLoading && !error && data?.transactions.length === 0 && (
        <div className="transactions-empty">
          ...
          <p>{t('sales.transactions.noTransactions') || 'No transactions found'}</p>
            ...
            >
              ...
              <div className="transaction-details">
                <div className="detail-row">
                  <span className="detail-label">{t('sales.transactions.date') || 'Date'}</span>
                  <span className="detail-value">{formatDate(transaction.payment_date)}</span>
                </div>
                <div className="detail-row">
                  <span className="detail-label">{t('sales.transactions.method') || 'Method'}</span>
                  <span className="detail-value">{transaction.payment_method}</span>
                </div>
                {transaction.order && (
                  <div className="detail-row">
                    <span className="detail-label">{t('sales.transactions.order') || 'Order'}</span>
                    <span className="detail-value">{transaction.order.name}</span>
                  </div>
                )}
                {transaction.reference && (
                  <div className="detail-row">
                    <span className="detail-label">{t('sales.transactions.reference') || 'Reference'}</span>
