...
//     {}
//   );
//   // Load user-written values from localStorage on mount (per device)
//   const storageKey = `userWrittenValues_${device.macAddress}`;
  
//   const [userWrittenValues, setUserWrittenValues] = useState<{ [key: string]: any }>(() => {
...
//   // Save user-written values to localStorage whenever they change
//   React.useEffect(() => {
//     if (typeof window === 'undefined') return;
//     try {
...
//     // If user has written a value, show exactly what they wrote
//     if (userWrittenValues[char.uuid] !== undefined) {
//       return userWrittenValues[char.uuid];
//     }
    
//     // Otherwise, show what's read from device (or initial value)
...
//     // Also update display immediately with user's exact input
//     setUpdatedValues((prev) => ({
//       ...prev,
//       [activeCharacteristic.uuid]: value,
//     }));
...
//           // Read back the value to verify write succeeded
//           // Note: handleRead will keep showing user's written value if it exists
//           setTimeout(() => {
