      ...
      (data: string, responseCallback: (response: any) => void) => {
        try {
          ...
          if (topic === responseTopic) {
            ...
            // Check for ODOO_SYNC_ERROR in signals
            const signals = paymentResponse?.signals || 
                           paymentResponse?.data?.signals || 
                           paymentResponse?.metadata?.signals || 
                           [];
            
            const hasOdooSyncError = Array.isArray(signals) && signals.includes("ODOO_SYNC_ERROR");
            
            if (hasOdooSyncError) {
              console.error("ODOO_SYNC_ERROR detected in payment check response");
              toast.error(t("Payment synchronization error. Please try again or contact support."));
            }
