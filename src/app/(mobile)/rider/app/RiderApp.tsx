...
interface RiderAppProps {
  /**
   * When false, hides the self-service energy Top-Up entry point (the home CTA
   * and the modal it opens). Used by the lightweight "Rider Basic" variant
   * (slug `rider-basic`) for SAs that handle energy top-ups through the
   * standalone Energy Top-Up applet instead. Defaults to true, so the canonical
   * `/rider/app` route keeps its existing behaviour unchanged.
   */
  showTopUp?: boolean;
}

const RiderApp: React.FC<RiderAppProps> = ({ showTopUp = true }) => {
  ...
  const fetchCustomerIdentificationData = async (planId: string, options?: { keepLoading?: boolean }) => {
    ...
    try {
      ...
      // can belong to a different region than the plan (a Kenyan rider attached
      // to Togo terms reads back "XOF"), so mixing the two put two different
      // currencies on one screen. Odoo prices these plans in the same currency
      // the plan record carries, so this keeps the whole app on one value.
      const billingCurrency = service_plan_data?.currency || '';
        ...
        >
          ...
          {currentScreen === 'plans' && (
            <RiderPlans
              ...
              onSelectPlan={(plan) => {
                // Show the human-readable product name to the user. The
                // `templateId` (x_template_id) is for lookups/filtering only —
                // never display it.
                const planName = plan.name || plan.templateId || '';
