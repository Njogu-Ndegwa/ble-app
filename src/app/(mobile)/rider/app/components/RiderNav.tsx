...
interface RiderNavProps {
  currentScreen: 'home' | 'map' | 'activity' | 'profile';
  onNavigate: (screen: 'home' | 'map' | 'activity' | 'profile') => void;
}
...
const RiderNav: React.FC<RiderNavProps> = ({ currentScreen, onNavigate }) => {
  return (
    <nav className="rider-nav">
      ...
      <button 
        className={`rider-nav-item ${currentScreen === 'map' ? 'active' : ''}`}
      ...
      >
        ...
        <span className="rider-nav-label">Stations</span>
      </button>
      ...
      >
        ...
        <span className="rider-nav-label">Activity</span>
      </button>
