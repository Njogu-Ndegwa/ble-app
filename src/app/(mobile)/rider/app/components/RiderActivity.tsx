...
import RiderActivityDetail from "./RiderActivityDetail";
...
export default function RiderActivity({
  ...
  // Row tapped by the rider — switches this screen to the full-page detail
  // view (RiderActivityDetail) until the rider taps back.
  ...
  // Full-page detail takes over the whole screen area — no overlay, so the
  // bottom nav can never clip it (the old bottom sheet was cut off by the nav,
  // a sibling stacking context of the `.rider-main` scroller).
  if (detail) {
    return (
      <RiderActivityDetail
        item={detail}
