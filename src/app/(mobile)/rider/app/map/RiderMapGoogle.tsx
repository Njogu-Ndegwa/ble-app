 ...
 *
 * This is the DEFAULT backend, used everywhere Google is reachable. Inside
 * mainland China (where every `*.googleapis.com` domain is blocked by the
 * Great Firewall) `RiderMap` switches to `RiderMapLeaflet` instead — see
 * `RiderMap.tsx` and `isChina.ts`.
 *
 ...
 * camera.
 *
 * Expects an ancestor `<RiderMapProvider>` (in `RiderMap.tsx`) to provide the
 * Google Maps JS runtime. Safe to mount/unmount freely — the heavy library
 * init happens once in that provider.
 */
...
export default function RiderMapGoogle(props: RiderMapProps) {
