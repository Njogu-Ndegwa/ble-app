...
export default function SwapHistory({ onSelectSwap }: SwapHistoryProps) {
  return (
<div className="min-h-screen bg-gradient-page p-4 flex items-center justify-center">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 flex justify-between items-center">
          <h1 className="text-xl font-semibold text-text-primary">Swap History</h1>
          <div className="w-6 h-6 flex items-center justify-center">
            <div className="space-y-1">
              <span className="block w-1 h-1 bg-bg-elevated rounded-full"></span>
              <span className="block w-1 h-1 bg-bg-elevated rounded-full"></span>
              <span className="block w-1 h-1 bg-bg-elevated rounded-full"></span>
            </div>
          </div>
        ...
        <div className="px-6 py-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600"
            />
            <Filter className="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-text-secondary" />
          </div>
        </div>
        ...
        <div className="px-6 py-4 flex justify-between">
          <div className="bg-purple-50 rounded-lg p-4 w-1/2 mr-2">
            ...
            <p className="text-sm text-text-muted">Total Swaps</p>
          </div>
        ...
        <div className="px-6 py-4">
          <h2 className="text-lg font-semibold text-text-primary mb-4">Recent Swaps</h2>
          <div className="space-y-4">
            {swapData.map((swap) => (
              <div
                key={swap.id}
                className="p-4 bg-bg-secondary rounded-lg hover:bg-bg-tertiary cursor-pointer"
                onClick={() => onSelectSwap(swap)}
              >
                <div className="flex justify-between items-center">
                  <div>
                    <div className="font-medium text-text-primary">{swap.riderName}</div>
                    <div className="text-xs text-text-muted">ID: {swap.id}</div>
                    <div className="text-sm text-text-muted mt-1">{swap.date}</div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="text-sm font-medium text-text-primary">{swap.pricing.currencySymbol} {swap.pricing.total.toFixed(1)}</span>
                ...
                <div className="flex items-center mt-2 space-x-2">
                  ...
                  <span className="text-sm text-text-muted">{swap.depletedBattery.percentage}%</span>
                  ...
                  <span className="text-sm text-text-muted">{swap.newBattery.percentage}%</span>
                </div>
                <div className="mt-2">
                  <span
