...
//       );
//       if (mapKey) {
//         return itemImageMap[mapKey] || defaultImageUrl;
//       }
//     }
//     return defaultImageUrl;
//   };

//   const convertRssiToFormattedString = (rssi: number, txPower: number = -59, n: number = 2): string => {
//     const distance = Math.pow(10, (txPower - rssi) / (10 * n));
//     return `${rssi}db ~ ${distance.toFixed(0)}m`;
//   };
...
//       setupBridge(bridge);
//       readDeviceInfo();
//       // Stop any potential BLE scanning on initialization
//       if (window.WebViewJavascriptBridge) {
//         window.WebViewJavascriptBridge.callHandler('stopBleScan', '', () => {
//           console.log('BLE scanning stopped on initialization');
//         });
//       }
//     }
//   }, [bridge]);
...
//     } else {
//       console.error("WebViewJavascriptBridge is not initialized.");
//       toast.error("QR code scanning is not available.");
//     }
//   };

//   const handleQrCode = (qrVal: string) => {
//     // Assuming QR code contains the MAC address directly or a value that can be used to construct it
//     const macAddress = qrVal.slice(-6).toLowerCase(); // Adjust based on actual QR code format
//     const deviceName = `OVES Device ${macAddress}`; // Construct a default name
//     const newDevice: BleDevice = {
//       macAddress,
//       name: deviceName,
//       rssi: convertRssiToFormattedString(-59), // Default RSSI, since no BLE scan
//       rawRssi: -59,
//       imageUrl: getImageUrl(deviceName),
//     };

//     setDetectedDevices((prev) => {
//       const exists = prev.some((device) => device.macAddress === macAddress);
//       if (exists) {
...
//     // Automatically Bose start connection to the scanned device
...
//       setSelectedDevice(macAddress);
//     } else {
//       if (connectedDevice && window.WebViewJavascriptBridge) {
//         window.WebViewJavascriptBridge.callHandler('disconnectBle', connectedDevice, () => {
//           console.error(`Disconnected from ${connectedDevice}`);
//           toast.error(`Disconnected from ${connectedDevice}`);
//           setConnectedDevice(null);
//           setServiceAttrList([]);
//           setAtrrList([]);
//         });
...
//     setLoadingService(serviceName);
//     setProgress(0);
//     const data = { serviceName, macAddress: selectedDevice };
//     initServiceBleData(data);
//   };
...
//   const handleBLERescan = () => {
//     // Since BLE scanning is disabled, clear devices and prompt for QR scan
//     setConnectedDevice(null);
//     setDetectedDevices([]);
//     setSelectedDevice(null);
//     setConnectingDeviceId(null);
//     toast('Please scan a QR code to add a device');
//   };
...
//   const convertRssiToFormattedString = (
//     rssi: number,
//     txPower: number = -59,
//     n: number = 2
//   ): string => {
