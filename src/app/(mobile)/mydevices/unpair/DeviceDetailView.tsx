...
const DeviceDetailView: React.FC<DeviceDetailProps> = ({
  ...
  useEffect(() => {
    const fetchAndCloseAssetAccount = async () => {
      if (!itemId) {
        console.log('Item ID not yet available, skipping fetchAndCloseAssetAccount');
        return;
      }
      ...
      if (!authToken) {
        toast.error('Please sign in to fetch asset accounts', { duration: 5000 });
        router.push('/signin');
        return;
      }
      ...
      const query = `
        query {
          getAllAssetAccountsForCustomer(first: 1, clientId: "${distributorId}", customerId: "${customerId}") {
            page {
              edges {
                node {
                  _id
      ...
      try {
        ...
        const assetAccountId = result.data.getAllAssetAccountsForCustomer.page.edges[0]?.node._id;
        if (!assetAccountId) {
          throw new Error('No asset account ID returned in response');
        }

        console.log('Asset account ID fetched successfully:', assetAccountId);
        toast.success('Asset account ID fetched successfully', { duration: 3000 });
        ...
        const closeResponse = await fetch(apiUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${authToken}`,
        ...
        if (!closeResponse.ok) {
          throw new Error(`HTTP error! Status: ${closeResponse.status}, Message: ${JSON.stringify(closeResult)}`);
        }

        if (closeResult.errors) {
          throw new Error(`GraphQL error: ${closeResult.errors.map((e: { message: any }) => e.message).join(', ')}`);
        }

        const { message, status } = closeResult.data.closeAssetAccount;
        console.log('Asset account closed successfully:', { message, status });
        toast.success(`Asset account closed successfully: ${message}`, { duration: 3000 });

      } catch (error) {
        console.error('Error in fetchAndCloseAssetAccount:', error);
        toast.error(
          `Failed to process asset account: ${error instanceof Error ? error.message : 'Unknown error'}`,
          { duration: 5000 }
        );
      }
    };

    fetchAndCloseAssetAccount();
  }, [itemId, router]);
  ...
  return (
    <div className="max-w-md mx-auto bg-gradient-to-b from-[#24272C] to-[#0C0C0E] min-h-screen text-white">
      ...
      <div className="p-4 flex items-center">
        ...
        <h1 className="text-lg font-semibold flex-1">Device Unpairing</h1>
