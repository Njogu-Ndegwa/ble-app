...
const TopupApp = dynamic(() => import('./TopupApp'), {
...
export default function TopupPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[TopupPage] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/topup" />
      )}
      {screen === 'selectSA' && (
      ...
      {screen === 'app' && <TopupApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
