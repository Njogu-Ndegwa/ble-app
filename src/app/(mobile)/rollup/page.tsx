...
const RollupApp = dynamic(() => import('./RollupApp'), {
...
export default function RollupPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[RollupPage] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/rollup" />
      )}
      {screen === 'selectSA' && (
        <SelectServiceAccount
          accounts={saAccounts}
      ...
      {screen === 'app' && <RollupApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
