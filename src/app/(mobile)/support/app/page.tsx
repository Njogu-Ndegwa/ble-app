...
const SupportApp = dynamic(() => import('./SupportApp'), {
...
export default function SupportPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[Support] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/support/app" />
      )}
      {screen === 'selectSA' && (
      ...
      {screen === 'app' && <SupportApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
