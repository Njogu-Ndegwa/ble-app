...
export default function TicketingPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[Ticketing] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/ticketing/app" />
      )}
      {screen === 'selectSA' && (
      ...
      {screen === 'app' && <TicketingApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
