...
import { Clock, FileText, Play, Phone, Mail, Eye } from 'lucide-react';
import ListScreen, { type ListPeriod } from '@/components/ui/ListScreen';

interface ActivatorSessionsProps {
  onSelectSession?: (order: OrderListItem, isReadOnly: boolean) => void;
}

const ITEMS_PER_PAGE = 15;

const ActivatorSessions: React.FC<ActivatorSessionsProps> = ({ onSelectSession }) => {
  const { t } = useI18n();
    ...
    try {
      ...
      const sessionsWithData = (response.orders || []).filter(
        order => order.session && order.session.session_data &&
          order.session.session_data.workflowType === 'activator'
      );
  ...
  return (
    <ListScreen
      title={t('activator.sessions.title') || 'Activation Sessions'}
      searchPlaceholder={t('activator.sessions.searchPlaceholder') || 'Search by subscription code...'}
      ...
      emptyMessage={
        searchQuery
          ? (t('activator.sessions.noSearchResults') || 'No sessions match your search')
          : (t('activator.sessions.noSessions') || 'No activation sessions found')
      }
      emptyHint={
        searchQuery
          ? (t('sales.sessions.tryDifferentSearch') || 'Try a different search term')
          : (t('activator.sessions.noSessionsHint') || 'Sessions will appear here after activations')
      }
...
export default ActivatorSessions;
