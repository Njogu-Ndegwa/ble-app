...
const AssemblyApp = dynamic(() => import('./AssemblyApp'), {
...
export default function AssemblyPage() {
  ...
  const loadServiceAccounts = useCallback(async (token: string) => {
    ...
    try {
      ...
      console.error('[Assembly] Failed to load SAs:', err);
  ...
  return (
    <>
      ...
      />
      {screen === 'login' && (
        <Login onLoginSuccess={handleLoginSuccess} userType="sales" microsoftReturnPath="/assembly" />
      )}
      {screen === 'selectSA' && (
      ...
      {screen === 'app' && <AssemblyApp onLogout={handleLogout} onSwitchSA={handleSwitchSA} />}
    </>
  );
}
