...
// Create a bridge provider component
...
useEffect(() => {
    if (progress === 100 && attributeList.length > 0) {
     ...
     // Critical fix: Only navigate to device detail if we were connecting
      ...
      setAtrrList(attributeList)
      console.error(attributeList, "Attribute List -----441----")
  ...
  // Critical fix: Make sure handleBackToList always sets selectedDevice to null
  const handleBackToList = () => {
    // Force the navigation back to list view with one state change
    setSelectedDevice(null);
    // Reset any connecting state to be sure
...
// Create a custom hook to use the bridge context
