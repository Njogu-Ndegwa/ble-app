...
export async function getSubscriptionProducts(
  ...
  try {
    ...
    // Read the body as text so a non-JSON response can be described precisely
    ...
    try {
      ...
      throw new Error(
        `Server returned invalid JSON (HTTP ${response.status}, ${rawText.length} bytes).`,
      );
    }
    ...
    if (rawData.categories) {
      ...
      // Not an auth problem. A real auth failure arrives as success === false
      // and is thrown above with the server's own message; reaching here means
      // a successful response that simply carried no catalog, so telling the
      // user to sign out and back in sends them somewhere that cannot help.
      const apiMessage = (rawData as any).message;
      throw new Error(
        apiMessage ||
          'The product catalog came back empty. Check that this service account has products assigned.',
      );
    }
